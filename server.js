...
/* ----------------------------------- /api/oms-probe (TEMPORARY diagnostic, read-only)
   Tests whether order addresses/weights hydrate (with= param, single-order detail) and
   whether the contact endpoint carries structured addresses + lat/lng. Masks all PII —
   returns only populated/empty booleans, key names, and structure. Remove after use. */
function omsAuthGet(p) {
  const url = `${OMS_API_BASE}${p}`;
  return fetch(url, { headers: { Authorization: `Bearer ${OMS_API_KEY}`, Accept: "application/json" } })
    .then(async (r) => ({ status: r.status, ok: r.ok, body: await r.json().catch(() => null) }));
}
const isFilled = (v) => v !== null && v !== undefined && !(typeof v === "string" && v.trim() === "") && !(Array.isArray(v) && v.length === 0);
function fieldReport(obj, keys) {
  const out = {};
  for (const k of keys) out[k] = isFilled(obj ? obj[k] : undefined) ? "FILLED" : "empty";
  return out;
}
app.post("/api/oms-probe", async (req, res) => {
  ...
  const ADDR_KEYS = ["deliveryAddress", "destinationAddress", "invoiceAddress", "street", "city", "postalCode", "houseNumber"];
  const out = { tests: {} };
  try {
    // varied sample: first page + a deep page, across statuses/services
    const p1 = await omsAuthGet("/v1/orders?per_page=25&page=1");
    const list = Array.isArray(p1.body) ? p1.body : (p1.body && p1.body.data) || [];
    out.tests.list_sample = {
      status: p1.status, n: list.length,
      sample_keys: list[0] ? Object.keys(list[0]) : [],
      per_order: list.slice(0, 12).map((o) => ({
        nr: o.order_nr, service: o.serviceType && o.serviceType.name, status: o.status,
        addr: fieldReport(o, ADDR_KEYS),
        items: Array.isArray(o.orderItems) ? o.orderItems.length : (o.order_items ? o.order_items.length : "no orderItems key"),
        weight_inline: isFilled(o.netto_weight || o.nettoWeight || o.weight) ? "FILLED" : "empty",
      })),
    };
    const uuid = list.find((o) => o.uuid)?.uuid || list[0]?.uuid;
    const contactUuid = list.find((o) => o.contact_uuid)?.contact_uuid || list[0]?.contact_uuid;
    // with / include param variants on non-admin orders
    const variants = [
      "?per_page=3&with=deliveryAddress,destinationAddress,orderItems.weightMeasurements,orderItems.wasteNumber",
      "?per_page=3&include=deliveryAddress,destinationAddress,orderItems.weightMeasurements",
      "?per_page=3&with[]=deliveryAddress&with[]=destinationAddress&with[]=orderItems.weightMeasurements",
    ];
    out.tests.with_param = [];
    for (const v of variants) {
      const r = await omsAuthGet("/v1/orders" + v);
      const arr = Array.isArray(r.body) ? r.body : (r.body && r.body.data) || [];
      const o0 = arr[0] || {};
      out.tests.with_param.push({
        query: v, status: r.status,
        addr: fieldReport(o0, ["deliveryAddress", "destinationAddress"]),
        first_item_keys: Array.isArray(o0.orderItems) && o0.orderItems[0] ? Object.keys(o0.orderItems[0]) : "no orderItems",
        weightMeasurements: Array.isArray(o0.orderItems) && o0.orderItems[0] ? (isFilled(o0.orderItems[0].weightMeasurements) ? "FILLED" : "empty") : "n/a",
      });
    }
    // single-order detail
    if (uuid) {
      const d = await omsAuthGet("/v1/orders/" + uuid);
      const o = d.body && (d.body.data || d.body);
      out.tests.order_detail = {
        status: d.status, keys: o ? Object.keys(o) : [],
        addr: fieldReport(o, ADDR_KEYS),
        deliveryAddress_shape: o && o.deliveryAddress && typeof o.deliveryAddress === "object" ? Object.keys(o.deliveryAddress) : (o ? typeof (o && o.deliveryAddress) : "no order"),
      };
    } else out.tests.order_detail = { skipped: "no uuid in list" };
    // contact detail — the candidate live-map path (addresses[] + lat/lng)
    if (contactUuid) {
      const c = await omsAuthGet("/v1/contacts/" + contactUuid);
      const co = c.body && (c.body.data || c.body);
      const addrs = co && (co.addresses || co.address_list || []);
      out.tests.contact_detail = {
        status: c.status, keys: co ? Object.keys(co) : [],
        n_addresses: Array.isArray(addrs) ? addrs.length : "no addresses key",
        address_shape: Array.isArray(addrs) && addrs[0] ? Object.keys(addrs[0]) : [],
        first_address: Array.isArray(addrs) && addrs[0] ? {
          has_street: isFilled(addrs[0].street), has_city: isFilled(addrs[0].city), has_postal: isFilled(addrs[0].postalCode || addrs[0].postal_code),
          has_lat: isFilled(addrs[0].lat || addrs[0].latitude), has_lng: isFilled(addrs[0].lng || addrs[0].longitude),
          flags: { is_work: addrs[0].is_work, is_depot: addrs[0].is_depot, is_general: addrs[0].is_general },
        } : null,
      };
    } else out.tests.contact_detail = { skipped: "no contact_uuid in list" };
    res.json(out);
